...
class Robot():
    ...
    def __init__(self, tcp_host_ip, is_testing):
        ...
        self.maxVelocity = 1.
        self.maxForce = 200.
        self.workspace_limits = np.asarray([[-0.325-0.025, -0.15+0.025], [0.65-0.025, 0.825+0.025], [0.1, 0.41]]) # Cols: min max, Rows: x y z (define workspace limits in robot coordinates)
        self.heightmap_resolution = 0.00225 / 4
    ...
    def resetRobot(self, aperture):
        ...
        self.rob.set_tcp((0.015, 0.001, 0.33090, 0, 0, 0))
        time.sleep(.5)
#        self.gp_control(175) #184
#        self.fg_length_control('0')
        print("reset")
    ...
    def exec_dig_grasp(self, pos, rot_z, rot_y):
        ...
        self.rob.movel((pos[0]-eefPose[0],pos[1]-eefPose[1],0,0,0,0), acc=0.3, vel=0.8,relative=True)
        #self.rob.movel((0,0,pos[2]-eefPose[2],rot_z,0,0), acc=0.05, vel=0.05,relative=True)
        move = m3d.Transform((0,0,-(pos[2]-eefPose[2]),0,0,0))
        ...
        self.rob.movel((0,0,-pos[2]+eefPose[2],0,0,0), acc=0.2, vel=0.3,relative=True)
        grasp_success = input("Is grasp successful? 1/0: ")
        self.go_to_home()
        self.gp_control(175)
